...
button = DigitalInOut(board.USER_SW)
...
display = ST7789(display_bus, rotation=90, width=320, height=240, backlight_pin=backlight)


# SD Card setup
sd_cs = board.GP22
sdcard = sdcardio.SDCard(spi, sd_cs)
vfs = storage.VfsFat(sdcard)  # type: ignore
...
mp3s.append('/sd/Scarlet Fire2.mp3')
...
mp3stream = audiomp3.MP3Decoder(mp3_file)  # type: ignore


def tracktext(full_path_name, position):
    return full_path_name#.split('_')[position].split('.')[0]                    #FIXME
...
mixer = audiomixer.Mixer(voice_count=1,sample_rate=32000, channel_count=1 ,                        #FIXME
...
tape_tilegrid = displayio.TileGrid(
    tape_bitmap, pixel_shader=tape_bitmap.pixel_shader)
main_display_group.append(tape_tilegrid)


# song name label
song_name_text_group = displayio.Group(
    scale=3, x=90, y=44)  # text label goes in this Group
song_name_text = tracktext(mp3_filename, 2)
song_name_label = label.Label(
    terminalio.FONT, text=song_name_text, color=orange_bright)
...
artist_name_label = label.Label(
    terminalio.FONT, text=artist_name_text, color=orange_bright)
...
def change_track(tracknum):
    ...
    mp3stream_fc = audiomp3.MP3Decoder(mp3_file_fc)  # type: ignore
...
while True:
    ...
    #     volume_adjust = min(
    #         max((mixer.voice[0].level + (encoder_delta*0.005)), 0.0), 1.0)
    ...
    if play_state is True:  # if not stopped, auto play next song
        ...
        if not mixer.playing:
            ...
            mp3_file, mp3stream, song_name, artist_name, mp3_bytes = change_track(
                track_number)
